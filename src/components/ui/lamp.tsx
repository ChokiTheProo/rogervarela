...
interface LampContainerProps {
  ...
  className?: string;
  /** "default" = original brightness, "soft" = dimmed glow so it never overpowers content */
...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[420px] md:min-h-[600px] flex-col items-center justify-center overflow-hidden bg-background w-full rounded-md z-0",
        className
      )}
    >
      <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0">
        <motion.div
          ...
          whileInView={{ opacity: soft ? 0.55 : 1, width: "30rem" }}
        ...
        <div
          className={cn(
            "absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-primary blur-3xl",
            soft ? "opacity-20" : "opacity-50"
          )}
        ></div>
        <motion.div
          initial={{ width: "8rem" }}
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          className={cn(
            "absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-accent blur-2xl",
            soft && "opacity-50"
          )}
        ></motion.div>
        <motion.div
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          className={cn(
            "absolute inset-auto z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-primary",
            soft && "opacity-70"
          )}
        ></motion.div>
